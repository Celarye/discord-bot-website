...
<<<<<<< configuration-with-registry
type FormValue = string | number | boolean | FormValue[] | Record<string, FormValue>;
=======
type FormValue =
  | string
  | number
...
<<<<<<< configuration-with-registry
  (e: "save" | "add", settingsData?: SettingsFormData, environmentData?: EnvironmentFormData): void;
=======
  (
    e: "save" | "add",
...
const initializeSettingsForm = () => {
  ...
  Object.entries(properties).forEach(([key, property]: [string, Property]) => {
    ...
    if (property.type === 'string') {
      ...
      formData[key] = defaultValue && typeof defaultValue === 'object' ? { ...defaultValue as Record<string, FormValue> } : {};
      if (property.properties) {
        Object.entries(property.properties).forEach(([subKey, subProperty]: [string, Property]) => {
          ...
          if (objectValue[subKey] === undefined) {
            if (subProperty.type === 'string') {
              ...
              objectValue[subKey] = subProperty.default !== undefined ? Number(subProperty.default) : 0;
            } else if (subProperty.type === 'boolean') {
              if (subProperty.default === true || subProperty.default === false) {
...
<<<<<<< configuration-with-registry
      if (typeof value === 'boolean') {
        ...
        if (Array.isArray(value) && value.length > 0) {
          filteredSettings[key] = value;
        } else if (typeof value === 'object' && value !== null && Object.keys(value as Record<string, FormValue>).length > 0) {
          filteredSettings[key] = value;
...
<<<<<<< configuration-with-registry
    <DialogContent class="sm:max-w-[600px] max-h-[80vh] overflow-y-auto plugin-dialog-content">
      <DialogHeader>
        <DialogTitle class="plugin-dialog-text">
          {{ mode === "add" ? 'Configure' : 'Plugin Settings for' }} {{ pluginName }}
        </DialogTitle>
        <DialogDescription v-if="pluginName" class="plugin-dialog-text">
          <span v-if="mode === 'add'">Configure the plugin settings before installation.</span>
          <span v-else>Configure plugin settings and environment variables</span>
          <span v-if="hasDependencies" class="block mt-1 text-blue-600">
            <Download class="inline h-3 w-3 mr-1" />
            Dependencies ({{ dependencies!.map(dep => dep.name).join(', ') }}) will be installed automatically.
=======
    <DialogContent
      ...
      <form v-if="pluginName" class="space-y-4 py-4 plugin-dialog-content" @submit.prevent="handleSubmit">
        <div v-if="hasDependencies" class="p-3 bg-blue-50 rounded-md border border-blue-200">
          <div class="flex items-center gap-2 text-sm plugin-dialog-text">
            ...
            <span class="font-medium text-blue-800">Dependencies will be installed:</span>
          </div>
          <div class="mt-1 flex flex-wrap gap-1">
            <Badge v-for="dep in dependencies" :key="dep.name" variant="outline" class="text-xs plugin-dialog-text">
=======
      <form
        v-if="pluginName"
...
<<<<<<< configuration-with-registry
              ...
              'grid-cols-2': (hasSettings && !hasEnvironment) || (!hasSettings && hasEnvironment),
...
<<<<<<< configuration-with-registry
            <TabsTrigger v-if="hasSettings" value="settings" class="plugin-dialog-text">
              <Settings class="h-4 w-4 mr-2" />
              Settings
            </TabsTrigger>
            <TabsTrigger v-if="hasEnvironment" value="environment" class="plugin-dialog-text">
=======
            <TabsTrigger
              v-if="hasSettings"
              value="settings"
...
<<<<<<< configuration-with-registry
              <Label for="version" class="text-right plugin-dialog-text">Version</Label>
              <div class="col-span-3">
                <Select v-model="localSelectedVersion">
                  <SelectTrigger :disabled="isLoading" class="plugin-dialog-text">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent class="plugin-dialog-content">
                    <SelectItem v-for="version in versions" :key="version" :value="version" class="plugin-dialog-text">
=======
...
<<<<<<< configuration-with-registry
                <p v-if="isLoading" class="text-xs text-muted-foreground mt-1 plugin-dialog-text">
                  Loading available versions...
                </p>
                <p v-else-if="versions.length === 0" class="text-xs text-muted-foreground mt-1 plugin-dialog-text">
=======
                <p
                  v-if="isLoading"
...
<<<<<<< configuration-with-registry
            ...
            >
              No configuration options available for this plugin. It will be installed with default settings.
=======
              v-if="!hasSettings && !hasEnvironment && mode === 'add'"
              class="text-sm text-muted-foreground p-4 bg-gray-50 rounded-md plugin-dialog-text"
            >
...
>>>>>>> master
          ...
          <TabsContent v-if="hasSettings" value="settings" class="space-y-4">
            <div class="space-y-4">
<<<<<<< configuration-with-registry
              <h4 class="text-sm font-medium plugin-dialog-text">Plugin Settings</h4>
              ...
              <div
                  ...
                  class="space-y-2"
              >
                <Label :for="key" class="text-sm font-medium plugin-dialog-text">
=======
...
<<<<<<< configuration-with-registry
                ...
                <div v-else-if="getFieldType(property) === 'checkbox'" class="flex items-center space-x-2">
                  <input
                      :id="key"
                  ...
                  >
                  <Label :for="key" class="text-sm plugin-dialog-text">{{ property.description || formatLabel(key) }}</Label>
                </div>

                <div v-else-if="getFieldType(property) === 'array'" class="space-y-2">
                  <div
                      v-for="(item, index) in (settingsFormData[key] as FormValue[] || [])"
                      :key="index"
                  ...
                  >
                    <Input
                        ...
                        @update:model-value="(value) => updateArrayItem(key, index, value)"
                    />
                    <Button
                        type="button"
                        variant="ghost"
...
<<<<<<< configuration-with-registry
                ...
                <div v-else-if="getFieldType(property) === 'object'" class="space-y-3 pl-4 border-l-2 border-gray-200">
                  <div
                      v-for="[subKey, subProperty] in Object.entries(property.properties || {})"
                      :key="`${key}.${subKey}`"
                      class="grid grid-cols-4 items-center gap-4"
                  >
                    <Label :for="`${key}.${subKey}`" class="text-xs text-muted-foreground text-right plugin-dialog-text">
                      {{ formatLabel(subKey) }}
                    </Label>
                    <Input
                        :id="`${key}.${subKey}`"
                        :model-value="(settingsFormData[key] as Record<string, FormValue>)?.[subKey] as string || ''"
                        :placeholder="subProperty.description || `Enter ${formatLabel(subKey)}`"
                        class="col-span-3 text-sm plugin-dialog-input"
                        @update:model-value="(value) => {
                        if (!settingsFormData[key]) settingsFormData[key] = {};
                        (settingsFormData[key] as Record<string, FormValue>)[subKey] = value;
                      }"
=======
                    type="button"
                    variant="outline"
...
<<<<<<< configuration-with-registry
                <p v-if="property.description" class="text-xs text-muted-foreground plugin-dialog-text">
=======
                <p
...
<<<<<<< configuration-with-registry
          <TabsContent v-if="hasEnvironment" value="environment" class="space-y-4">
            <div class="space-y-4">
              <div class="flex items-center gap-2">
                <Zap class="h-4 w-4 text-yellow-600" />
                <h4 class="text-sm font-medium plugin-dialog-text">Environment Variables</h4>
              </div>
              <p class="text-xs text-muted-foreground plugin-dialog-text">
                Configure environment variables for this plugin. Variables prefixed with "env." will be resolved from system environment.
              </p>
              ...
              >
                ...
                <div v-else-if="getEnvironmentFieldType(value) === 'checkbox'" class="flex items-center space-x-2">
                  <input
                      :id="`env-${key}`"
            ...
            <div class="space-y-4">
              ...
              >
                ...
                <p class="text-xs text-muted-foreground plugin-dialog-text">
                  Current:
                  <code
                    class="bg-gray-100 px-1 rounded text-xs plugin-dialog-text"
                    >{{ String(value) }}</code
                  >
>>>>>>> master
                </p>
        ...
        <DialogFooter>
<<<<<<< configuration-with-registry
          <Button type="button" variant="outline" class="plugin-dialog-text" @click="closeDialog">Cancel</Button>
          <Button type="submit" class="plugin-dialog-text" :disabled="isLoading">

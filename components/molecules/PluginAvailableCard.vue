...
// Since plugins don't have multiple versions in your metadata, create a simple version structure
...
const availableVersions = computed(() => {
  // For now, just return the current version since your metadata doesn't have multiple versions
  return [currentVersion.value.version];
});

// Check if plugin has settings that can be configured
const hasSettings = computed(() => {
  if (!props.plugin.settings) return false;

  // Handle both schema format and direct properties
...
// Check if plugin has environment variables that can be configured
...
const handleSimpleInstall = (): void => {
  // Convert environment to proper format (strings only)
...
const handleSettingsDialogSubmit = (settingsData?: Record<string, unknown>, environmentData?: Record<string, string>): void => {
  ...
  // Ensure environment data is properly formatted as strings
...
// Get environment requirements info
...
// Prepare settings and environment for the dialog
...
const dialogEnvironment = computed(() => {
  ...
  // Convert environment to string format for the dialog
  const env = props.plugin.environment as Record<string, boolean | string>;
  const environmentWithValues: Record<string, string> = {};

  Object.entries(env).forEach(([key, value]) => {
    if (value === true) {
      // Required field with no default - use empty string

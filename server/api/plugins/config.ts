    ...
    try {
        ...
        try {
            ...
            if (metadataResponse.ok) {
                ...
                // Environment schema: { "API_KEY": true } means API_KEY is required
                if (metadata.environment && typeof metadata.environment === 'object' && !Array.isArray(metadata.environment)) {
                    ...
                    // Create default environment object with empty strings for required fields
                    Object.entries(metadata.environment).forEach(([key, required]) => {
                        if (required) {
                            defaultEnvironment[key] = ''; // Required fields get empty string as default
                        }
                    });
                }
    ...
    if (method === "POST") {
        try {
            ...
            if (body.action === 'add-plugin') {
                ...
                // Merge user-provided values with defaults from registry
